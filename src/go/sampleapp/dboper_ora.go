...
func postEmployeeOra(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	//
...
func checkDBObjectOra(db *sql.DB, objname string) (int, error) {
	...
	var cnt int
	err := db.QueryRow("select count(*) from user_tables where table_name==:1", objname).Scan(&cnt)
	if err != nil {
...
func initDBOra(db *sql.DB) error {
	...
	if os.Getenv("DBVERSION") == "ORACLE" || dbversion == "ORACLE" {
		//
		chk, chkerr := checkDBObjectOra(db, "employee")
		if chkerr != nil {
			log.Fatal(chkerr)
		}
		if chk == 0 {
